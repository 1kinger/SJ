...
(function($) {
    ...
    function RenderData(data, element){
        for(var i = 0 ; i< data.length ; i++){
            ...
            if(!element.is("li")){
                element.append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="far fa-circle select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                ...
                element.find("ul").append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="far fa-circle select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                if(data[i].data != null && typeof data[i].data !="undefined"){
                    $("#TreeElement"+globalTreeIdCounter).append("<ul style='display:none'></ul>");
                    $("#TreeElement"+globalTreeIdCounter).find("a").first().prepend('<span class="arrow">'+options.closedArrow+'</span>');
                    RenderData(data[i].data, $("#TreeElement"+globalTreeIdCounter).find("ul").first());
    ...
    $(options.element).on("click",".select-box",function(e){
        ...
        if($(this).hasClass("fa-circle")){
            ...
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").removeClass("fa-circle");
                $(this).parents("li").first().find(".select-box").addClass("fa-check-circle");
            }
        }else{
            //will unselect
            checked=false;
            ...
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").addClass("fa-circle");
                $(this).parents("li").first().find(".select-box").removeClass("fa-check-circle");
                $(this).parents("li").each(function(){
                    $(this).find(".select-box").first().removeClass("fa-check-circle");
                    $(this).find(".select-box").first().addClass("fa-circle");
                });
            }
        }
...
cat_MENY=document.getElementsByClassName("dropdown-menu")[0];
zemi_cat(cat_MENY);
    ...
    $(options.element).init.prototype.GetSelected = function(title){
        ...
        $(this).find(".fa-check-circle").each(function(){
