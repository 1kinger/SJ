...
//		console.log("Selected Elements",$("#example").GetSelected());
	...
	closedArrow: '<i class="fas fa-angle-double-right" aria-hidden="true">  </i>',
	openedArrow: '<i class="fas fa-angle-double-down"  aria-hidden="true">  </i>',
	multiSelect: true
}
...
function zemi_cat(a){ 
br_GRANKA=a.getElementsByClassName("arrow"); selectS="";N=0;N_max=0;
for(i=0;i<br_GRANKA.length;i++){
ARROW= a.getElementsByClassName("arrow")[i]; GRANKA=ARROW.parentElement.nextElementSibling;
...
for(item=0;item<GRANKA.childElementCount;item++){ N_max=N_max+1;
  if(GRANKA.children[item].children[0].className.includes("-check-")){
   ...
   if(document.getElementById("j_cat")!=undefined){puniCAT(document.getElementById("j_cat"));}
   
   }}
   
N_select(N,N_max);}

function N_select(a,b){
document.getElementsByClassName("dropdowntree-name")[0].innerHTML="Selected ( "+a+" / "+b+" )";}

function puniCAT(a){// samo za userot
	a.value=CAT_SELECT;
	a.innerHTML=CAT_SELECT;}


////////////////////////// ova ebavase pola sat 

function CEKIRAJ_cat(a){defalNULA();var M=null; 
if(a){ M=decodiraj(a); Stikliraj(M)};}


function defalNULA(){cat_MENY=document.getElementsByClassName("dropdown-menu")[0];	
N_granka=cat_MENY.childElementCount;
for(i=0;i<N_granka;i++){  /// ova gi stiska site
	cat_MENY.children[i].children[0].click();}
	
for(x=0;x<N_granka;x++){
	if(cat_MENY.children[x].children[0].classList==("far select-box fa-check-circle")){
		cat_MENY.children[x].children[0].click();}}
	
}

function decodiraj(a){ QWE=[[],[],[],[],[],[],[],[],[],[],[]];/// da se ubacat br na granki spored ova sega ima 11 kategori
for(I=0;I<11;I++){
  for(i=0;i<11;i++){
    bara="C"+I+"-"+i;
     if(a.includes(bara)){ QWE[I].push(i);}}} return QWE;}
	 
	
function Stikliraj(a){ cat_MENY=document.getElementsByClassName("dropdown-menu")[0];
for(I=0;I<a.length;I++){
   if(a[I].length){
   cat_MENY.children[I].children[0].classList.toggle("fa-circle");
   cat_MENY.children[I].children[0].classList.add("fa-check-circle"); 
   
   for(i in a[I]){ 
	cat_MENY.children[I].children[2].children[i].children[0].click();}}
